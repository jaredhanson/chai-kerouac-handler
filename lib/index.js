...
exports = module.exports = function(chai, utils) {
  ...
  Assertion.addMethod('options', function(options) {
    var obj = this._obj;
    
    new Assertion(obj).to.be.an.instanceof(Page);
    new Assertion(obj).to.have.property('_renderOptions');
    
    var assertion = new Assertion(obj._renderOptions);
    utils.transferFlags(this, assertion, false);
    assertion.deep.equal(options);
  });
  ...
  Assertion.addMethod('format', function(format) {
    var obj = this._obj;
    
    new Assertion(obj).to.be.an.instanceof(Page);
    new Assertion(obj).to.have.property('_format');
    
    this.assert(
        obj._format == format
      , 'expected page to compile format #{exp} but compiled #{act}'
      , 'expected page to not compile format #{act}'
      , format  // expected
      , obj._format  // actual
    );
  });
